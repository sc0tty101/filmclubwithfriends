...
app.get('/', (req, res) => {
  ...
  function isCurrentWeek(weekDate) {
    const currentMonday = getMondayOfWeek(new Date());
    const currentWeekStr = formatDate(currentMonday);
    return weekDate === currentWeekStr;
  }
  ...
  function isNearFuture(weekDate) {
    const currentMonday = getMondayOfWeek(new Date());
    const threeWeeksOut = new Date(currentMonday);
    threeWeeksOut.setDate(currentMonday.getDate() + (3 * 7));
    const threeWeeksOutStr = formatDate(threeWeeksOut);
    
    return weekDate > formatDate(currentMonday) && weekDate <= threeWeeksOutStr;
  }

  // 🔧 FIXED: New function to generate weeks based on database range
  function generateWeeksFromDatabase(earliestDate, latestDate) {
    const weeks = [];
    ...
    // Ensure we cover from earliest DB date to at least 48 weeks in the future
    const futureLimit = new Date(currentMonday);
    futureLimit.setDate(currentMonday.getDate() + (48 * 7));
    
    // Use the later of database max date or future limit
    const actualEndDate = endDate > futureLimit ? endDate : futureLimit;
    
    // Generate weeks from start to end
    let currentWeekDate = new Date(startDate);
    
    while (currentWeekDate <= actualEndDate) {
      const weekDateStr = formatDate(currentWeekDate);
      
      weeks.push({
        date: weekDateStr,
        displayDate: currentWeekDate.toLocaleDateString('en-US', { 
          weekday: 'long', 
          year: 'numeric', 
          month: 'long', 
          day: 'numeric' 
        }),
        shortDate: currentWeekDate.toLocaleDateString('en-US', { 
          month: 'short', 
          day: 'numeric',
          year: '2-digit'
        }),
        isCurrentWeek: isCurrentWeek(weekDateStr),
        isPastWeek: isPastWeek(weekDateStr),
        isNearFuture: isNearFuture(weekDateStr),
        isFarFuture: !isPastWeek(weekDateStr) && !isCurrentWeek(weekDateStr) && !isNearFuture(weekDateStr)
      });
  ...
  db.all("SELECT name FROM members WHERE is_active = 1 ORDER BY name", (err, members) => {
    ...
    // 🔧 FIXED: Get the date range from database first
    db.get("SELECT MIN(week_date) as earliest, MAX(week_date) as latest FROM weeks", (err, dateRange) => {
      if (err) {
        console.error(err);
        return res.status(500).send('Database error');
      }

      // Generate weeks based on database range (or default if no data)
      let weeks;
      if (dateRange && dateRange.earliest && dateRange.latest) {
        weeks = generateWeeksFromDatabase(dateRange.earliest, dateRange.latest);
      } else {
        ...
        `, (err, weekStats) => {
          ...
          `, (err, userActivity) => {
            ...
            `, (err, winners) => {
              ...
              // 🔧 FIXED: Split weeks using actual date logic, not generated flags
                ...
                <body>
                  ...
                  <div class="container">
                    <div class="header">
                      ...
                      <p class="date-range-info">
                        📅 Showing ${weeksData.length} weeks 
                        ${dateRange && dateRange.earliest ? `(${dateRange.earliest} to ${formatDate(new Date(Date.now() + (48 * 7 * 24 * 60 * 60 * 1000)))})` : ''}
                      </p>
                    </div>
                    ...
                    ${pastWeeks.length > 0 ? `
                      <div class="section-header">
                        <h2>📅 Past Weeks (${pastWeeks.length})</h2>
                        <button onclick="toggleSection('pastWeeksList')" class="toggle-btn">Show/Hide</button>
                      </div>
                      <div id="pastWeeksList" class="compact-weeks" style="max-height: 400px; overflow-y: auto;">
                        ${pastWeeks.slice(-20).map(week => renderCompactWeek(week)).join('')}
                        ${pastWeeks.length > 20 ? `<div class="more-indicator">... and ${pastWeeks.length - 20} more past weeks</div>` : ''}
                      </div>
                    ` : ''}
                    ...
                    ${farFutureWeeks.length > 0 ? `
                      <div class="section-header">
                        <h2>🔮 Future Schedule (${farFutureWeeks.length})</h2>
                        <button onclick="toggleSection('farFutureWeeksList')" class="toggle-btn">Show/Hide</button>
                      </div>
                      <div id="farFutureWeeksList" class="compact-weeks" style="display: none; max-height: 600px; overflow-y: auto;">
                  ...
                  function renderCurrentWeekFilms(films) {
                    if (films.length === 1) {
                      ...
                      return '<div class="current-week-grid">' + films.map(film => renderFilmCardCompact(film)).join('') + '</div>';
                    }
                    return '';
                  }
                  ...
                  function renderFilmShowcase(film) {
                    ...
                    return '<div class="film-showcase">' +
                      ...
                      '<div class="film-content">' +
                        ...
                        '<div class="film-details-large">' +
                          '<h2 class="film-title-large">' + film.film_title + ' <span class="film-year-large">(' + film.film_year + ')</span></h2>' +
                          ...
                          '</div>' +
                          (film.overview ? '<div class="film-overview-large">' + film.overview + '</div>' : '') +
                          '<div class="film-credits">' +
                            (film.director ? '<div class="film-director"><strong>Director:</strong> ' + film.director + '</div>' : '') +
                            '<div class="film-nominator"><strong>Nominated by:</strong> ' + film.user_name + '</div>' +
                          '</div>' +
                        '</div>' +
                      '</div>' +
                    '</div>';
                  ...
                  function renderFilmCardCompact(film) {
                    ...
                    return '<div class="film-card-compact">' +
                      ...
                      '<div class="film-card-content">' +
                        ...
                        '</div>' +
                        '<div class="film-card-nominator">Nominated by ' + film.user_name + '</div>' +
                      '</div>' +
                    '</div>';
                  }
